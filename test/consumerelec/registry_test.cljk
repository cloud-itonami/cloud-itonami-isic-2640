(ns consumerelec.registry-test
  (:require [clojure.test :refer [deftest is]]
            [consumerelec.registry :as r]))
...
(deftest known-product-types-are-valid
  (doseq [pt [:television :audio-device :video-device
              :smart-speaker :wearable-device]]
...
;; ----------------------------- dielectric-test-kv-valid? -----------------------------

(deftest typical-dielectric-test-kv-is-valid
  (is (r/dielectric-test-kv-valid? 0.0))
  (is (r/dielectric-test-kv-valid? 1.5))
  (is (r/dielectric-test-kv-valid? 3.0))
  (is (r/dielectric-test-kv-valid? 5.0)))

(deftest negative-dielectric-test-kv-is-invalid
  (is (not (r/dielectric-test-kv-valid? -1.0))))

(deftest excessive-dielectric-test-kv-is-invalid
  (is (not (r/dielectric-test-kv-valid? 999999.0)))
  (is (not (r/dielectric-test-kv-valid? 5.01))))

(deftest non-numeric-or-missing-dielectric-test-kv-is-invalid
  (is (not (r/dielectric-test-kv-valid? nil)))
  (is (not (r/dielectric-test-kv-valid? "3.0"))))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "smt-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "smt-001" 1)
