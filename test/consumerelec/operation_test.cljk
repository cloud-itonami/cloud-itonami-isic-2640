(ns consumerelec.operation-test
  "Smoke tests for the compiled ConsumerElecOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `consumerelec.governor-contract-test`; the Store contract in
  `consumerelec.store-contract-test`."
...
(deftest test-actor-builds
  (testing "ConsumerElecOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "smt-001" :severity :moderate :description "battery pack overheating anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
