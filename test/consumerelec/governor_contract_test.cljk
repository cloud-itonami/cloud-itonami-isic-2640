(ns consumerelec.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control SMT/assembly/test-bench equipment
  directly... does NOT authorize or execute maintenance/equipment
  operations... does NOT self-issue a consumer-electronics safety-
  certification mark') implemented faithfully. The single invariant
  under test:

    ConsumerElecAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Consumer Electronics Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :television}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :television (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-smt-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the consumer-electronics safety-certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-dielectric-test-kv-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:dielectric-test-kv 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-dielectric-test-kv} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:dielectric-test-kv (store/batch db "batch-001"))) "fabricated dielectric-test-kv never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "smt-001" :severity :moderate
                                            :description "battery pack thermal anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "smt-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
