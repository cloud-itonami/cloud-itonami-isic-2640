(ns consumerelec.registry
  "Pure-function domain logic for the consumer-electronics
  plant-operations coordination actor -- equipment/batch
  verification, shipment-quantity recompute, product-type validation,
  dielectric (hipot/withstand) safety-test-voltage plausibility
  ...
  coordinated shipment), not the act of actuating SMT/assembly/
  test-bench equipment or dispatching a real freight carrier, and
  never the act of issuing a consumer-electronics safety-certification
  mark (e.g. UL/CE/FCC/RoHS) (this actor NEVER does any of those --
  see README `What this actor does NOT do`).

  SCOPE: ISIC 2640 covers manufacture of consumer electronics --
  surface-mount-technology (SMT) PCB assembly (component placement,
  reflow soldering), final assembly, and functional/safety test lines
  producing finished consumer-electronics products (televisions,
  audio/video devices, smart speakers, wearable devices). This actor
  ...
  flagging, shipment coordination) -- it never touches the SMT/
  assembly/test-bench equipment directly, and it never stands in for
  the certification body that issues consumer-electronics
  safety/RoHS/FCC/CE compliance marks.")
...
(def dielectric-test-kv-min
  "Physical floor for a batch's own dielectric (hipot/withstand)
  safety-test voltage reading, in kV. A test voltage is never
  negative."
  0.0)

(def dielectric-test-kv-max
  "Physical ceiling for a batch's own dielectric (hipot/withstand)
  safety-test voltage reading, in kV. Grounded in IEC 62368-1's
  electric-strength (hipot) test-voltage tables for mains-connected
  audio/video/ICT equipment, which top out around 4.2 kV peak for
  reinforced insulation on the highest standard consumer-electronics
  working-voltage class -- a reading above this is implausible
  sensor/QC data, not a real routine dielectric test on any
  standardized class of consumer-electronics equipment."
  5.0)

(def defect-rate-min-percent
  "Physical floor for a batch's own SMT/assembly/test defect-rate
  reading (zero defects is the best possible outcome, never negative)."
  0.0)

(def defect-rate-max-percent
  "Physical ceiling for a batch's own SMT/assembly/test defect-rate
...
(defn batch-verified?
  ...
  (i.e. its product-type/dielectric-test-kv/quantity/defect-rate
...
(defn dielectric-test-kv-valid?
  "Is `kv` a physically plausible dielectric (hipot/withstand)
  safety-test voltage reading, in kV? Rejects nil, non-numbers,
  negative values, and values beyond `dielectric-test-kv-max` -- a
  fabricated or sensor-error reading, never let through as a real
  test-result fact."
  [kv]
  (and (number? kv)
       (>= (double kv) dielectric-test-kv-min)
       (<= (double kv) dielectric-test-kv-max)))

(defn defect-rate-valid?
  "Is `percent` a physically plausible batch SMT/assembly/test
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER a consumer-electronics safety-certification mark
  -- this actor is never the certification authority (see README
...
(defn register-maintenance
  ...
  SMT/assembly/test-bench-equipment maintenance window against a
  verified, registered piece of equipment. Pure function -- does not
  actuate the SMT/assembly/test-bench equipment or execute any
  maintenance; it builds the RECORD a plant coordinator would keep.
  `consumerelec.governor` independently re-verifies the equipment's own
...
(defn register-shipment
  ...
  outbound consumer-electronics product shipment against a verified,
  ...
  keep. `consumerelec.governor` independently re-verifies the
  shipment's own claimed quantity against `shipment-quantity-exceeded?`,
