(ns consumerelec.advisor
  "ConsumerElecAdvisor -- the *contained intelligence node* for the
  consumer-electronics plant-operations coordination actor.

  It normalizes production-batch patches (product-type/dielectric-
  test-kv/quantity/defect-rate), drafts an SMT/assembly/test-bench-
  ...
  consumer-electronics product shipment coordination proposal against
  a production batch. CRITICAL: it is a smart-but-untrusted advisor.
  It returns a *proposal* (with a rationale + the fields it cited),
  never a committed record and NEVER a real SMT/assembly/test-bench-
  equipment actuation, freight dispatch, or safety-certification --
  see README `What this actor does NOT do`. Every output is censored
  downstream by `consumerelec.governor` before anything touches the
  SSoT.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `consumerelec.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct SMT/assembly/
  ...
  `consumerelec.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [consumerelec.registry :as registry]
...
(defn- log-production-batch
  ...
  dielectric-test-kv, quantity, defect-rate or verification status.
...
(defn- schedule-maintenance
  "Draft an SMT/assembly/test-bench-equipment maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `consumerelec.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft a battery-safety/electrical-safety/RoHS-compliance concern.
  ...
  and it is never gated on the referenced equipment/batch being
  verified (a concern can be raised about ANY equipment or batch,
  verified or not -- see README `What this actor does NOT do` re:
  never blocking safety-relevant reporting on an administrative
  technicality). See `consumerelec.phase`: no phase ever adds this op
  to a phase's `:auto` set; `consumerelec.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound consumer-electronics product shipment coordination
  ...
  `consumerelec.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは消費者向け電子機器(テレビ・オーディオ・映像機器・"
       "スマートスピーカー・ウェアラブル機器)製造工場"
       ...
       "SMT実装・組立・試験等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "消費者向け電子機器安全認証(UL/CE/FCC/RoHS等)を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `consumerelec.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :consumerelec-advisor-proposal
   :op         (:op request)
