(ns consumerelec.operation
  "ConsumerElecOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (ConsumerElecAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Consumer Electronics Plant Operations Governor (:govern) and the
  ...
  (:require [langgraph.graph :as g]
            ...
            [consumerelec.advisor :as advisor]
            [consumerelec.governor :as governor]
            [consumerelec.phase :as phase]
...
(defn build
  "Compiles a ConsumerElecOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `consumerelec.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ConsumerElecAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Consumer Electronics Plant Operations Governor -- independent censor.
      (g/add-node :govern
