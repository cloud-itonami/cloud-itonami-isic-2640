(ns consumerelec.store
  "SSoT for the consumer-electronics plant-operations coordination
  ...
  `elecequipmfg.store`, `cloud-itonami-isic-2013`'s own
  `resinmfg.store`), this build ships a single `MemStore` backend only
  ...
  self-contained (no external consumer-electronics-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/dielectric-test-kv/quantity/
    ...
    - `equipment`         -- an SMT/assembly/test-bench unit's own
    ...
    - `maintenance`       -- a scheduled SMT/assembly/test-bench
                             maintenance-window DRAFT against a piece
                             of equipment (`consumerelec.registry`'s
    ...
    - `shipments`         -- a proposed outbound consumer-electronics
                             product shipment DRAFT
                             (`consumerelec.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (consumerelec.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (consumerelec.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :television
                :model "TV-55-4K" :dielectric-test-kv 3.0
                :quantity-units 500.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-07-01"}
   "batch-002" {:id "batch-002" :product-type :smart-speaker
                :model "SPK-100" :dielectric-test-kv 1.5
                :quantity-units 200.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 180.0
                :last-assessed "2026-07-01"}
   "batch-003" {:id "batch-003" :product-type :wearable-device
                :model "WRB-20" :dielectric-test-kv 0.5
...
(defn- sample-equipment []
  {"smt-001" {:id "smt-001" :kind :smt-placement-line
              :verified? true :registered? true
              :last-maintenance-date "2026-06-01"}
   "final-test-002" {:id "final-test-002" :kind :final-assembly-test-bench
                      :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `consumerelec.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `consumerelec.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  SMT placement-line unit (schedulable for maintenance), one
  UNVERIFIED/unregistered final-assembly test-bench unit (blocks any
