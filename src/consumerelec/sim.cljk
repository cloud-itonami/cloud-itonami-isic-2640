(ns consumerelec.sim
  ...
  production quantity, a proposal that tries to ACTUATE SMT/assembly/
  test-bench equipment directly (permanently blocked, no override), a
  proposal that tries to self-issue a consumer-electronics safety-
  certification mark (permanently blocked, no override), a double-
  schedule of the same maintenance window, a production-batch patch
  with a fabricated product type, a production-batch patch with an
  implausible dielectric-test-kv reading, and a production-batch patch
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on smt-001 (verified, registered, SMT placement line -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on smt-001 (always escalates -- approve) ==")
    ...
    (println (exec-op actor "t5"
                       {:op :log-production-batch :effect :direct-write :subject "batch-001"
                        :patch {:product-type :television}}
                       coordinator))

    (println "== unrecognized op -> HARD hold ==")
    (println (exec-op actor "t6"
                       {:op :actuate-smt-line :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on final-test-002 (UNVERIFIED/unregistered test bench -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on smt-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    (println (exec-op actor "t10"
                       {:op :schedule-maintenance :effect :propose :subject "mnt-3"
                        :value {:equipment-id "smt-001" :maintenance-type :force-run
    ...
    (println "== log-production-batch batch-001 with an implausible dielectric-test-kv -> HARD hold ==")
    ...
    (println "== log-production-batch batch-001 attempting to self-issue a consumer-electronics safety certification -> HARD hold, PERMANENT ==")
